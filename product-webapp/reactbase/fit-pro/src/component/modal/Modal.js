...
const Modal = ({ onRequestClose, id }) => {
	...
	useEffect(() => {
		...
		const getAllSlots = () => {
			...
			fetch(`https://fitpro.stackroute.io/appointmentservice/api/slot/expertslot/${id}`)
		.then(res=>res.json())
	...
	const bookAppointment = (item) => {
		...
		const FetchFunc =
    axios.get(`https://fitpro.stackroute.io/userservice/api/v1/expertprofile/${item.expertId}`)
      .then((res) => res.json())
      .then((response) => 
        setExpertProfile(response)
      )
      .catch((err) => {});
    const res = axios.post("https://fitpro.stackroute.io/appointmentservice/api/appointment/save", {
      	appointmentId: "",
		userEmailId: emailId,
		...
		const updateSlot = axios.put("https://fitpro.stackroute.io/appointmentservice/api/slot/update", {
