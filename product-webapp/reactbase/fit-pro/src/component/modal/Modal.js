...
const Modal = ({ onRequestClose, id }) => {
	...
	useEffect(() => {
		...
		const getAllSlots = () => {
			...
			fetch(`http://13.58.188.1/appointmentservice/api/slot/expertslot/${id}`)
		.then(res=>res.json())
	...
	const bookAppointment = (item) => {
		...
		const FetchFunc =
    axios.get(`http://13.58.188.1/userservice/api/v1/expertprofile/${item.expertId}`)
      .then((res) => res.json())
      .then((response) => 
        setExpertProfile(response)
      )
      .catch((err) => {});
    const res = axios.post("http://13.58.188.1/appointmentservice/api/appointment/save", {
      	appointmentId: "",
		userEmailId: emailId,
		...
		const updateSlot = axios.put("http://13.58.188.1/appointmentservice/api/slot/update", {
