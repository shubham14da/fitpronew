...
function Appointment() {
  ...
  const FetchFunc = () =>
    fetch(`http://3.16.66.107:8080/appointmentservice/api/slot/expertslot/${emailId}`)
      .then((res) => res.json())
      .then((response) => {
        setData(response)
      })
  ...
  const handleAdd = (e) => {
    ...
    const res = axios.post("http://3.16.66.107:8080/appointmentservice/api/slot/newslot", {
  ...
  const handleCancel = (e) => {
    const deleteSlot = axios.delete(`http://3.16.66.107:8080/appointmentservice/api/slot/delete/${e}`
    ).then(res => {
      FetchFunc()
    })
    .catch(err => {});
  ...
  const handleSubmit = (e) => {
    ...
    //   }
    // }
    const res = axios.put(`http://3.16.66.107:8080/appointservice/api/slot/update`, updateData, {
      headers: {
