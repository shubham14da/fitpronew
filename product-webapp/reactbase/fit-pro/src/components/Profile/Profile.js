...
function Profile() {
  ...
  const myfun = () => {
    fetch(`http://${window.location.hostname}:8080/userservice/api/v1/expertprofile/${email}`)
      .then((res) => res.json())
      .then(
        (response) => (
  ...
  const handleSubmit = (e) => {
    ...
    const res = axios.put(`http://${window.location.hostname}:8080/userservice/api/v1/expert/${email}`, {
      firstName: firstName,
      lastName: lastName,
